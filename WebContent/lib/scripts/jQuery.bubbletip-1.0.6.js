...
; (function($) {
	...
	$.fn.extend({
		bubbletip: function(tip, options) {
			...
			// handle window.resize REMOVED FOR EFFICIENCY
/*			$(window).bind('resize.bubbletip' + _bindIndex, function() {
			...
			if(_options.bindShow == 'click')
				...
				ba = [_wrapper.get(0),this.get(0)];

			// handle mouseover and mouseout events
			$(ba).bind(_calc.bindShow, function() {
				if (_timeoutAnimate) {
			...
			//alert(this.get(0))
			// handle mouseover and mouseout events
			$(_wrapper.get(0)).bind('mouseover', function() {
				if (_timeoutAnimate) {
					clearTimeout(_timeoutAnimate);
				}
				if (_options.delayShow === 0) {
			...
			if(_options.bindHide != 'click')
			{
			//	ba = [this.get(0)];
			//else
				ba = [_wrapper.get(0),this.get(0)];
				
			$(ba).bind(_calc.bindHide, function() {
												//alert('hide')
				if (_timeoutAnimate) {
					clearTimeout(_timeoutAnimate);
				}
			...
			//enhanced to accomodate for another element doing the close with the specified event
			if(_options.hideElements != null)
			{
				for(var q = 0; q <_options.hideElements.length; q++ )
				{
					$(_options.hideElements[q]).bind(_calc.bindHide, function() {
						if (_timeoutAnimate) {
							clearTimeout(_timeoutAnimate);
						}
						if (_options.delayHide === 0) {

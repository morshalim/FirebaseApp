 ...
 * @version 1.2-SNAPSHOT ($Id: jquery.numberformatter.js,v 1.1 2014/09/10 17:36:22 shariramulu Exp $)
 *
...
(function(jQuery) {
	...
	jQuery._formatNumber = function(number, options, suffix, prefix, negativeInFront) {
		...
		if (options.format.indexOf(".") > -1) {
			...
			//custom logic added to round to decimalToRound can be specified in the options.
			if (options.round == true)
			{
				var df = decimalFormat.length;
				if(options.decimalToRound != null)
					df = options.decimalToRound;

				if(options.roundRule != null && options.roundRule.roundInc != null) {
					options.roundRule.roundInc = parseInt(options.roundRule.roundInc);
					if(options.roundRule.roundRule == null || options.roundRule.roundRule == "STD") //standard rounding
						number = Math.round(number/options.roundRule.roundInc) * options.roundRule.roundInc;
					else if(options.roundRule.roundRule == "DOWN") //round down
						number = Math.floor(number/options.roundRule.roundInc) * options.roundRule.roundInc;
					else if(options.roundRule == "UP")
						number = Math.ceil(number/options.roundRule.roundInc) * options.roundRule.roundInc;
				} else if(options.roundRule != null && options.roundRule.roundRule != null) {
					var shiftBy = (df*10);
					if(options.roundRule.roundRule == "DOWN") 
						number = Math.floor(shiftBy * val) / shiftBy;
					else if(options.roundRule.roundRule == "UP") 
						number = Math.ceil(shiftBy * val) / shiftBy;
					
				}
				//if anything has happened with the options above really the next line will not affect the number at all
				number = new Number(number.toFixed(df));
